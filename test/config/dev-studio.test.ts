import type { DrizzleModuleHost } from '../../src/nitro-module/context'
...
/** Minimal host shape `resolveDrizzleModuleContext` reads. */
function fakeHost(
  dev: boolean,
  studio: unknown = { studioUrl: 'local.drizzle.studio' },
): DrizzleModuleHost {
  return {
    drizzle: {
      dialect: 'sqlite',
...
describe('studio resolution in module context', () => {
  ...
  it('resolves and validates the studio only alongside the dev database', async () => {
    ...
    await expect(resolveDrizzleModuleContext(fakeHost(true)))
      .rejects
  ...
  it('mints the per-session localhost domain for the runtime', async () => {
    ...
    // When
    const context = await resolveDrizzleModuleContext(fakeHost(true, {}))
  ...
  it('ignores an invalid studio config in production builds', async () => {
    ...
    const context = await resolveDrizzleModuleContext(fakeHost(false))
